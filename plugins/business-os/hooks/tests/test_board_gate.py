...
def main():
    ...
    case("status to בעבודה on a returned task", 0, run(CONNECTOR + "update-page", {"page_id": "p", "properties": {"סטטוס": "בעבודה", "תוצאה": "סבב 2: נלקח"}}))
    case("result quotes the founder's reply", 0, run(CONNECTOR + "update-page", {"page_id": "p", "properties": {"תוצאה": "סבב 2: קוצר לפי מה שבעז כתב בתגובת מייסד"}}))
    ...
    case("status to לסבב נוסף", 2, run(CONNECTOR + "update-page", {"page_id": "p", "properties": {"סטטוס": "לסבב נוסף"}}))
    case("write תגובת מייסד", 2, run(CONNECTOR + "update-page", {"page_id": "p", "properties": {"תגובת מייסד": "תקצר את זה"}}))
    case("write תגובת מייסד alongside an allowed field", 2, run(CONNECTOR + "update-page", {"page_id": "p", "properties": {"סטטוס": "בעבודה", "תגובת מייסד": ""}}))
    case("clear תגובת מייסד", 2, run(CONNECTOR + "update-page", {"page_id": "p", "properties": {"תגובת מייסד": None}}))
    case("create a row with a founder reply", 2, run(CONNECTOR + "create-pages", {"parent": board, "pages": [{"properties": {"תגובת מייסד": "בסדר"}}]}))
    ...
    case("copy the founder's reply into the page body", 0, run(REST + "patch-block-children", {"block_id": "p", "children": [{"heading_2": {"rich_text": [{"text": {"content": "סבב 2"}}]}}, {"paragraph": {"rich_text": [{"text": {"content": "תגובת מייסד (17.9): קצר מדי, תרחיב"}}]}}]}))
    ...
    case("patch תגובת מייסד as rich text", 2, run(REST + "patch-page", {"page_id": "p", "properties": {"תגובת מייסד": {"rich_text": [{"text": {"content": "לא זה"}}]}}}))
    case("patch תגובת מייסד with niqqud in the name", 2, run(REST + "patch-page", {"page_id": "p", "properties": {"תְּגוּבַת מְיַיסֵּד": {"rich_text": []}}}))
    case("patch Founder Note", 2, run(REST + "patch-page", {"page_id": "p", "properties": {"Founder Note": {"rich_text": [{"text": {"content": "x"}}]}}}))
    case("patch תגובת מייסד by property id", 2, run(REST + "patch-page", {"page_id": "p", "properties": {"תגובת מייסד (BOS)": {"rich_text": []}}}))
