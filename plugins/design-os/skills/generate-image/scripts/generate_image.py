...
def missing_key_message(problem=None):
    ...
    if COWORK_UPLOADS.parent.is_dir() and not problem:
        return message + (
            ...
            "את הקובץ שהועלה לבד. אין לך device_stage_files — חסם בראש הדיווח: מפתח התמונות לא הועלה לענן, "
            "המנכ\"ל מעלה אותו ומריץ שוב. אין קובץ כזה בתיקייה המחוברת — חסם בראש הדיווח. "
